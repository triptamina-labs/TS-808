...
const Knob = props => {
  const { value, min, max, step, size, onChange, bufferSize } = getKnobNormalizedProps(props);
  ...
  const onMoveShouldSetPan = React.useCallback(
    (evt, gestureState) => {
      ...
      if (rootElem != null) {
        ...
        const topPos =
          pageY - (BASE_HEIGHT * scale - getNormalizedValue(value, min, max) * (BASE_HEIGHT * scale));
  ...
  const onPanMove = React.useCallback(
    (evt, gestureState) => {
      ...
      const cursorPosition = [pageX, pageY];
      let normalizedValue = (100 - (tempY - topPosition) * (100 / (BASE_HEIGHT * scale))) / 100;
    ...
    [handleUnormalizedChange, knobCenter, knobId, max, min, setOverlayState, step, topPosition]
  );
  ...
  const handleAccessibleValueUpdate = React.useCallback(
    evt => {
      switch (knobType) {
        case "select": {
          let processedValue = isValueNumber ? parseFloat(evt.target.value) : evt.target.value;
  ...
  const accessibilityElement = React.useMemo(() => {
    switch (props.type ?? "range") {
      case "select": {
        ...
        const optionElements = options.map(({ value: optVal, displayName }) => {
          return (
      ...
      case "range": {
        const { value: v, min: mn, max: mx, step: st } = props;
        return (
          <input
            ref={accessibilityElementRef}
            type="range"
