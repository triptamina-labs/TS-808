...
import VCO, { SINE, TRIANGLE } from "synth/basics/vco";
...
export default function claveRimshot(audioCtx, destination, time, { level, selector }) {
  // Parameters.
  ...
  const claveFilter =
    selector === 0
      ? (() => {
          ...
          return new VCF(BANDPASS, audioCtx);
        })()
      : (() => {
          claveOsc.frequency.value = RIM_CLAVE_FREQ;
          return new VCF(HIGHPASS, audioCtx);
        })();
